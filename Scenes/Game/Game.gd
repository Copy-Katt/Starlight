...
#	if Global.fill_num(Global.score, 8) != $UI/Score.text:
	$UI/Score.text = "%09d" % Global.score #Global.fill_num(Global.score, 9)
#	if Global.lifes != $UI/LifesBar.value:
	$UI/LifesBar.value = Global.lifes
	timer += _delta
	$UI/Timer.text = "%02d:%02d:%03d" % [int(floor(timer) / 60), fmod(floor(timer), 60), fmod(floor(timer*1000), 1000)]
